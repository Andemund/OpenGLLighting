...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	//gui_1->addVariable("object color:", object_color);
...
void Renderer::display(GLFWwindow* window)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (temp_sr != sr)
		{
			...
			{
				m_shader = Shader("./shader_flat/basic.vert", "./shader_flat/basic.frag");
			}
			else
			{
				std::cout << "Wrong shading type." << std::endl;
			}
			temp_sr = sr;
		}
		//my code end
...
{
	//my coed start
	//m_lightings->point_light.position[0] = m_camera->position[0];
	//m_lightings->point_light.position[1] = m_camera->position[1];
	//m_lightings->point_light.position[2] = m_camera->position[2];
	//my code end
